...
export function WavyLines() {
  ...
  const svgString = useMemo(
    () => buildWaveSvg(style, fgColor, bgColor, amplitude, frequency, strokeWidth, lineCount, seed),
    [style, fgColor, bgColor, amplitude, frequency, strokeWidth, lineCount, seed]
  )

  const svgDataUrl = useMemo(() => `data:image/svg+xml,${encodeURIComponent(svgString)}`, [svgString])
    ...
    <div className="flex flex-col gap-6 h-full">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Generate flowing sine waves, coils, ripples and oscillating lines</p>
        </div>
      ...
      <div className="flex gap-5 flex-1 min-h-0">
        ...
        <div className="flex flex-col gap-5 w-56 shrink-0 overflow-y-auto pr-1">
          ...
          {[
            { label: "Amplitude", value: amplitude, set: setAmplitude, min: 5, max: 120 },
            { label: "Frequency", value: frequency, set: setFrequency, min: 1, max: 12, step: 0.5 },
            { label: "Stroke Width", value: strokeWidth, set: setStrokeWidth, min: 0.5, max: 6, step: 0.5 },
            { label: "Line Count", value: lineCount, set: setLineCount, min: 3, max: 60 },
          ...
          <div className="flex flex-col gap-2">
            ...
            <input type="range" min={0} max={999} value={seed} onChange={(e) => setSeed(Number(e.target.value))}
              className="w-full accent-sky-600" />
          </div>
        </div>
        ...
        <div className="flex-1 rounded-xl border border-border overflow-hidden min-h-0">
          <img src={svgDataUrl} alt="Wave preview" className="w-full h-full object-cover" />
        </div>
      </div>
    </div>
  )
