...
import { Lock, Unlock, Copy, Check, Shuffle, Plus, Trash2, RefreshCw } from "lucide-react"
...
const SCHEMES = ["Analogous", "Triadic", "Complementary", "Monochrome"] as const
...
export function ColorPalette() {
  ...
  const generate = useCallback(() => {
    setSwatches((prev) => {
      ...
      let newIdx = 0
      return prev.map((s) => (s.locked ? s : { hex: newColors[newIdx++], locked: false }))
    })
  }, [])

  const applyScheme = useCallback((s: Scheme) => {
    setScheme(s)
    ...
    const buildHues = (): number[] => {
      switch (s) {
        case "Triadic":
          return Array.from({ length: count }, (_, i) => (hue + (i % 3) * 120) % 360)
        case "Complementary":
          return Array.from({ length: count }, (_, i) => (hue + (i % 2) * 180) % 360)
        case "Monochrome":
          return Array.from({ length: count }, () => hue)
        default: // Analogous
          return Array.from({ length: count }, (_, i) => (hue + i * 20) % 360)
      }
    }
  ...
  return (
    <div className="flex flex-col gap-6 h-full max-w-5xl mx-auto">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Generate or build your own palette — click any hex to edit it manually
          </p>
        </div>
        <div className="flex gap-2 flex-wrap">
          <Button variant="secondary" size="sm" onClick={copyAll} className="gap-2">
            {copiedAll ? <Check className="w-3.5 h-3.5 text-tool-palette" /> : <Copy className="w-3.5 h-3.5" />}
            ...
            >
              ...
              <div className="flex justify-between p-3 opacity-0 group-hover:opacity-100 transition-opacity">
                <button
                  ...
                  className={cn("w-7 h-7 rounded-full flex items-center justify-center transition-all bg-black/10 hover:bg-black/20", iconClass)}
                  title={swatch.locked ? "Unlock" : "Lock"}
                >
                  {swatch.locked ? <Lock className="w-3 h-3" /> : <Unlock className="w-3 h-3" />}
                </button>
                <div className="flex gap-1">
                  ...
                  {swatches.length > MIN_COLORS && (
                    <button
                      ...
                      className={cn("w-7 h-7 rounded-full flex items-center justify-center transition-all bg-black/10 hover:bg-red-500/60 hover:text-white", iconClass)}
                      title="Remove swatch"
                    >
              ...
              {swatch.locked && (
                <div className="flex justify-center">
                  <span className={cn("px-2 py-0.5 rounded-full text-xs", badgeClass)}>Locked</span>
                </div>
              )}
              ...
              <div className="p-4 flex flex-col gap-2">
                ...
                {editingIdx === idx ? (
                  <input
                    ...
                    className={cn(
                      ...
                      light ? "text-black/90 border-black/30 caret-black" : "text-white/90 border-white/30 caret-white"
                    )}
                    spellCheck={false}
                  />
                ) : (
                ...
                <p className={cn("font-mono text-xs", light ? "text-black/45" : "text-white/45")}>
                  {h}° {s}% {l}%
                </p>
              </div>
            </div>
      ...
      <p className="text-xs text-muted-foreground text-center">
        Click any hex label to edit it manually &middot; hover a swatch to lock, pick, copy, or remove
      </p>
    </div>
  )
}
